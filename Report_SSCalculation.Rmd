...
- **Significance Level (α)**: \( \alpha = \frac{0.05}{6} \) (adjusted for multiple comparisons)
...
- **Power**: 80%, 85%, and 90%
- **Missing Data**: 10% of the data is expected to be missing
...
results %>%
  kable("latex", caption = "Sample Size Results in Each group", position = "h!") %>%
...
- Bonferroni-adjusted α = 0.0083 for 6 comparisons
...
for (j in seq_along(Nvec)) {
  ...
  for (i in 1:n_simulations) {
    ...
    tx2 <- rnorm(N, mean = 50 + 2*delta, sd = sd_inflated)
    tx3 <- rnorm(N, mean = 50 + 3*delta, sd = sd_inflated)
    
    # Two-sided t-tests
    p1 <- t.test(control, tx1, var.equal = TRUE)$p.value
    p2 <- t.test(control, tx2, var.equal = TRUE)$p.value
    p3 <- t.test(control, tx3, var.equal = TRUE)$p.value
    
    # Correct approach: minimum p-value (not mean)
    significant[i] <- mean(c(p1, p2, p3)) < alpha
  }
